 ...
 *
 * M2 修正(C1 同源):簽發者必須是該 mandate 的預期 ECR 角色(M1=FAB財務主管、M2=Brand 永續長,
...
async function verifyAggregateForDisclosure(
  ...
  // vct↔issuer 綁定:pcf_aggregate 只認FAB LE AID,且以實際驗章鑰(sig.kid)為準,payload.iss 不得脫鉤。
...
/**
 * F4:閘道(FAB LE 鑰)對每次 PERMIT 簽出 receipt——綁 presentation_hash + mandate_jti +
